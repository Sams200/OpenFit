...
package org.openfit.util.fuzzySearch
...
object DiffUtils {
    ...
    fun levEditDistance(s1: String, s2: String, xcost: Int): Int {
        ...
        /* strip common prefix */while (len1 > 0 && len2 > 0 && c1[str1] == c2[str2]) {
            len1--
            len2--
            str1++
            str2++
        ...
        /* strip common suffix */while (len1 > 0 && len2 > 0 && c1[str1 + len1 - 1] == c2[str2 + len2 - 1]) {
            len1--
            len2--
        }

        /* catch trivial cases */if (len1 == 0) return len2
        if (len2 == 0) return len1

        /* make the inner cycle (i.e. str2) the longer one */if (len1 > len2) {
            val nx = len1
            val temp = str1
            len1 = len2
            len2 = nx
        ...
        /* check len1 == 1 separately */if (len1 == 1) {
            return if (xcost != 0) {
         ...
         * fast.  */if (xcost != 0) {
            ...
            while (i < len1) {
                ...
                /* skip the upper triangle */if (i >= len1 - half) {
                ...
                /* skip the lower triangle */if (i <= half + 1) end = len2 + i - half - 2
                /* main */while (p <= end) {
                ...
                /* lower triangle sentinel */if (i <= half) {
                    val c3 = --d + if (ch1 != c2[c2p]) 1 else 0
                    x++
                    if (x > c3) {
                        x = c3
